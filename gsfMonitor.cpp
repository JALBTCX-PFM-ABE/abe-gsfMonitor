...
gsfMonitor::gsfMonitor (int32_t *argc, char **argv, QWidget * parent):
...
{
  int32_t                 year, day, mday, month, hour, minute, gsf_handle = -1;
  float                   second;
  ...
  if ((abe_share->mwShare.multiType[0] == PFM_GSF_DATA && abe_share->key < INT32_MAX && l_share.nearest_point != abe_share->nearest_point) || force_redraw)
    {
      ...
      int32_t beam = l_share.mwShare.multiSubrecord[0];
      ...
      if (gsfRead (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING, &gsf_data_id, &gsf_record, NULL, 0) < 0) 
        {
          ...
          errmsg.sprintf (tr ("Error reading record %d : %s").toLatin1 (), gsf_data_id.record_number, gsfStringError ());
      ...
      double lat = gsf_record.mb_ping.latitude;
      double lon = gsf_record.mb_ping.longitude;
      double ang1 = gsf_record.mb_ping.heading + 90.0;
      double ang2 = gsf_record.mb_ping.heading;
      float dep = 0.0, nom_dep = 0.0;
          ...
          if (gsf_record.mb_ping.number_beams == 1)
          ...
          else
            {
              ...
              if (dep != 0.0 && gsf_record.mb_ping.beam_flags != NULL &&
                  !(check_flag (gsf_record.mb_ping.beam_flags[beam], NV_GSF_IGNORE_NULL_BEAM))) 
                {
                  //  Adjust for cross track position.  

                  double lateral = gsf_record.mb_ping.across_track[beam];
      ...
      month++;
      string.sprintf (tr ("<b>Date/Time : </b>%d-%02d-%02d (%03d) %02d:%02d:%05.2f<br>").toLatin1 (), year + 1900, month, mday, day,
                      hour, minute, second);
      listBox->insertHtml (string);

      strcpy (ltstring, fixpos (nxy.y, &deg, &min, &sec, &hem, POS_LAT, pos_format));
      string.sprintf (tr ("<b>Latitude : </b>%s<br>").toLatin1 (), ltstring);
      listBox->insertHtml (string);

      strcpy (lnstring, fixpos (nxy.x, &deg, &min, &sec, &hem, POS_LON, pos_format));
      string.sprintf (tr ("<b>Longitude : </b>%s<br>").toLatin1 (), lnstring);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Ping number : </b>%d<br>").toLatin1 (), gsf_data_id.record_number);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Beam number : </b>%d<br>").toLatin1 (), beam);
      listBox->insertHtml (string);

      if (gsf_record.mb_ping.depth != NULL)
        {
          string.sprintf (tr ("<b>Depth : </b>%f<br>").toLatin1 (), dep);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.nominal_depth != NULL)
        {
          string.sprintf (tr ("<b>Nominal Depth : </b>%f<br>").toLatin1 (), nom_dep);
          listBox->insertHtml (string);
        }

      string.sprintf (tr ("<b>Height above ellipsoid : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.height);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Ellipsoid/datum separation : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.sep);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Number of beams : </b>%d<br>").toLatin1 (), gsf_record.mb_ping.number_beams);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Center beam : </b>%d<br>").toLatin1 (), gsf_record.mb_ping.center_beam);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Ping flags : </b>%x<br>").toLatin1 (), gsf_record.mb_ping.ping_flags);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Tide corrector : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.tide_corrector);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>GPS Tide corrector : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.gps_tide_corrector);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Depth corrector : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.depth_corrector);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Heading : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.heading);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Pitch : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.pitch);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Roll : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.roll);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Heave : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.heave);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Course : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.course);
      listBox->insertHtml (string);

      string.sprintf (tr ("<b>Speed : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.speed);
      listBox->insertHtml (string);

      if (gsf_record.mb_ping.across_track != NULL)
        {
          string.sprintf (tr ("<b>Across track : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.across_track[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.along_track != NULL)
        {
          string.sprintf (tr ("<b>Along track : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.along_track[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.travel_time != NULL)
        {
          string.sprintf (tr ("<b>Travel time : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.travel_time[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.beam_angle != NULL)
        {
          string.sprintf (tr ("<b>Beam angle : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.beam_angle[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.mc_amplitude != NULL)
        {
          string.sprintf (tr ("<b>Mean calibrated amplitude : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.mc_amplitude[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.mr_amplitude != NULL)
        {
          string.sprintf (tr ("<b>Mean relative amplitude : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.mr_amplitude[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.echo_width != NULL)
        {
          string.sprintf (tr ("<b>Echo width : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.echo_width[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.quality_factor != NULL)
        {
          string.sprintf (tr ("<b>Simrad quality factor : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.quality_factor[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.receive_heave != NULL)
        {
          string.sprintf (tr ("<b>Receive heave : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.receive_heave[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.depth_error != NULL)
        {
          string.sprintf (tr ("<b>Depth error : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.depth_error[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.across_track_error != NULL)
        {
          string.sprintf (tr ("<b>Across track error : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.across_track_error[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.along_track_error != NULL)
        {
          string.sprintf (tr ("<b>Along track error : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.along_track_error[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.quality_flags != NULL)
        {
          string.sprintf (tr ("<b>Reson quality flags (hex) : </b>%x<br>").toLatin1 (), gsf_record.mb_ping.quality_flags[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.beam_flags != NULL)
        {
          string.sprintf (tr ("<b>Beam flags (hex) : </b>%x<br>").toLatin1 (), gsf_record.mb_ping.beam_flags[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.signal_to_noise != NULL)
        {
          string.sprintf (tr ("<b>Signal to noise : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.signal_to_noise[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.beam_angle_forward != NULL)
        {
          string.sprintf (tr ("<b>Beam angle forward : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.beam_angle_forward[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.vertical_error != NULL)
        {
          string.sprintf (tr ("<b>Vertical error : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.vertical_error[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.horizontal_error != NULL)
        {
          string.sprintf (tr ("<b>Horizontal error : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.horizontal_error[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.sector_number != NULL)
        {
          string.sprintf (tr ("<b>Transit sector number : </b>%d<br>").toLatin1 (), gsf_record.mb_ping.sector_number[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.detection_info != NULL)
        {
          string.sprintf (tr ("<b>Bottom detection method : </b>%d<br>").toLatin1 (), gsf_record.mb_ping.detection_info[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.incident_beam_adj != NULL)
        {
          string.sprintf (tr ("<b>Incident beam adjustment : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.incident_beam_adj[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.system_cleaning != NULL)
        {
          string.sprintf (tr ("<b>System data cleaining info : </b>%d<br>").toLatin1 (), gsf_record.mb_ping.system_cleaning[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.doppler_corr != NULL)
        {
          string.sprintf (tr ("<b>Doppler correction : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.doppler_corr[beam]);
          listBox->insertHtml (string);
        }

      if (gsf_record.mb_ping.brb_inten != NULL)
        {
          string.sprintf (tr ("<b>Bathymetric receive beam intensity : </b>%f<br>").toLatin1 (), gsf_record.mb_ping.brb_inten[beam]);
          listBox->insertHtml (string);
        }
    }
}
...
#ifdef NVWIN3X
  QString ini_file = QString (getenv ("USERPROFILE")) + "/ABE.config/gsfMonitor.ini";
#else
  QString ini_file = QString (getenv ("HOME")) + "/ABE.config/gsfMonitor.ini";
#endif

  QSettings settings (ini_file, QSettings::IniFormat);
  settings.beginGroup ("gsfMonitor");
